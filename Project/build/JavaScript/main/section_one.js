...
let OpenDisplay = (open, id) => {
    if (open) {
        serviceDisplay.classList.add('open');
        if (serviceImage != null)
            serviceImage.style.backgroundImage = `url(${cards[id].imageid}${imgTags.mediumTag}.png)`;
        if (serviceHeader != null)
            serviceHeader.textContent = cards[id].header;
        if (serviceParagraph != null)
            serviceParagraph.textContent = cards[id].description;
    }
    else {
