...
function DisplayHero(heroInfo) {
    if (displayName != null)
        displayName.textContent = heroInfo.name;
    if (displayUniverse != null)
        displayUniverse.textContent = '';
    for (let info in heroInfo.biography) {
        if (displayUniverse != null)
            displayUniverse.innerHTML += `<br> ${info}: ${heroInfo.biography[info]}`;
        console.log(info);
    }
    console.log(heroInfo.image.url);
    if (displayImage != null)
        displayImage.style.backgroundImage = `url('${heroInfo.image.url}')`;
}
